...
class WorkBuddyApp:
    ...
    def initialize_components(self) -> bool:
        ...
        try:
            ...
            # Initialize activity tracker with error handling
            try:
                self.activity_tracker = WindowsActivityTracker(self.db_manager, poll_interval=5)
                print("✅ Activity tracker initialized")
            except Exception as tracker_error:
                print(f"⚠️ Activity tracker initialization failed: {tracker_error}")
                print("   → WorkBuddy will continue without automatic app tracking")
                self.activity_tracker = None
    ...
    def _show_soundscape_menu(self) -> None:
        ...
        while True:
            try:
                ...
                print("7. Quick Play - Fireplace Crackling")
                print("8. Stop All Sounds")
                print("9. 🔄 Regenerate All Sounds (New & Improved!)")
                print("0. ← Back to Main Menu")
                print("─" * 50)
                ...
                if choice == '0':
                    ...
                    self._quick_play_sound('fireplace')
                elif choice == '8':
                    ...
                    if self.soundscape_manager:
                        self.soundscape_manager.regenerate_all_sounds()
                        print("🎵 Try playing the sounds now - they should be much more distinct!")
                    else:
    ...
    def _show_status(self) -> None:
        ...
        if self.activity_tracker:
            ...
            if status.get('is_tracker_disabled', False):
                print(f"Tracking: 🔒 Disabled (API errors: {status.get('api_error_count', 0)})")
                print("  → Activity tracking disabled to prevent Windows API errors")
                print("  → WorkBuddy wellness features still work normally")
            else:
                ...
                if status.get('api_error_count', 0) > 0:
                    print(f"API errors: {status['api_error_count']} (tracker will disable at 10)")
