...
class WindowsActivityTracker:
    def __init__(self, db_manager: DatabaseManager, poll_interval: int = 5):
        """Initialize Windows activity tracker."""
        ...
        self.api_error_count = 0  # Track consecutive API errors
        self.max_api_errors = 10  # Max errors before disabling
        self.tracker_disabled = False  # Flag to disable tracker if too many errors
    ...
    def get_active_window_info(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            # Get window title with proper error handling
            try:
                ...
                if length == 0 or length is None:
                    ...
                    result = windll.user32.GetWindowTextW(hwnd, buffer, length + 1)
                    if result > 0:
            ...
            return window_title or None, process_name
    ...
    def track_activity(self) -> None:
        ...
        while self.is_running and not self.tracker_disabled:
            try:
                # Add small delay to prevent overwhelming Windows API
                time.sleep(0.1)
                
                # Check if tracker should be disabled due to too many errors
                if self.api_error_count >= self.max_api_errors:
                    print(f"⚠️ Activity tracker disabled due to {self.api_error_count} consecutive Windows API errors")
                ...
                try:
                    ...
                    print(f"Idle detection error (continuing): {idle_error}")
                    self.api_error_count += 1
                    time.sleep(self.poll_interval)
                    continue
                
                # Get current active window with error handling
                try:
                    window_title, process_name = self.get_active_window_info()
                    # Reset error count on successful API call
                    self.api_error_count = 0
                except Exception as api_error:
                    print(f"Windows API error #{self.api_error_count + 1}: {api_error}")
                    self.api_error_count += 1
                    time.sleep(self.poll_interval)
                    continue
                ...
                print(f"Error in tracking loop (continuing): {e}")
                self.api_error_count += 1
                time.sleep(self.poll_interval)
        
        if self.tracker_disabled:
            print("🔒 Activity tracker is now disabled to prevent Windows API errors")
            print("   WorkBuddy will continue to work without automatic app tracking")
    ...
    def start_tracking(self) -> None:
        ...
        if not self.is_running:
            ...
            self.thread.start()
            print("Activity tracking started in background thread")
    
    def is_tracker_disabled(self) -> bool:
        """Check if tracker is disabled due to Windows API errors."""
        return self.tracker_disabled
    
    def get_error_count(self) -> int:
        """Get current API error count."""
    ...
    def get_current_status(self) -> Dict:
        ...
        # Safely check if system is idle
        is_idle = False
        ...
        return {
            ...
            "is_idle": is_idle,
            "is_tracker_disabled": self.is_tracker_disabled(),
            "api_error_count": self.get_error_count()
        }
