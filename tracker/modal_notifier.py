...
try:
    ...
    # Test if ToastNotifier works properly
    test_notifier = ToastNotifier()
    del test_notifier
except (ImportError, Exception) as e:
    print(f"Win10toast not available or has issues: {e}")
...
class ModalNotificationWindow:
    ...
    def __init__(self, title: str, message: str, notification_type: str = "info", 
        ...
        try:
            ...
            print(f"Error initializing modal window: {init_error}")
            # Create a minimal fallback window or raise the error to be handled by caller
            raise init_error
    ...
    def setup_window(self):
        ...
        try:
            ...
            # Make it modal and always on top with error handling
            try:
                ...
                print(f"Modal setup warning (continuing): {modal_error}")
                # Continue without modal properties if they fail
            ...
            # Set icon if available - with error handling
            try:
            ...
            print(f"Error in modal window setup: {e}")
            # Fallback to simple window setup
            try:
                self.root.geometry("600x400")
                self.root.title("WorkBuddy Reminder")
                self.root.configure(bg='#f0f0f0')
            except Exception as fallback_error:
                print(f"Fallback window setup failed: {fallback_error}")
...
class ModalNotificationManager:
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.modal_disabled = False  # Add flag to disable modal if it causes errors
    ...
    def show_modal_notification(self, title: str, message: str, 
        ...
        # If modal notifications are disabled due to previous errors, use fallback
        if self.modal_disabled:
            print(f"Modal disabled, using fallback for: {title}")
            return self._show_fallback_notification(title, message, notification_type)
        ...
        try:
            ...
            print(f"Modal notification failed, disabling modal notifications: {e}")
            # Disable modal notifications for future calls to prevent repeated errors
            self.modal_disabled = True
            return self._show_fallback_notification(title, message, notification_type)
    
    def _show_fallback_notification(self, title: str, message: str, notification_type: str) -> str:
        """Show fallback notification when modal fails."""
        try:
    ...
    def show_toast_notification(self, title: str, message: str, icon: str = None, duration: int = 10) -> None:
        ...
        try:
            if TOAST_AVAILABLE:
                ...
                # Fall back to console if toast not available
                self.show_console_notification(title, message)
        except Exception as e:
