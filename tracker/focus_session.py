...
class FocusSessionManager:
        ...
        if session_type == SessionType.FOCUS:
            ...
            if self.is_modal_notifier:
                try:
                    ...
                    print(f"Modal notification failed, using fallback: {modal_error}")
                    # Fallback to regular notifications if modal fails
                    self._show_fallback_focus_completion()
            else:
            ...
            if self.is_modal_notifier:
                try:
                    ...
                    print(f"Break modal notification failed, using fallback: {modal_error}")
                    # Fallback to regular notifications
                    self._show_fallback_break_completion()
            else:
                # Fallback to regular notifications
                self._show_fallback_break_completion()
    ...
    def _show_fallback_focus_completion(self):
        """Show fallback focus completion notification when modal fails."""
        ...
        if hasattr(self.notifier, 'show_toast_notification'):
            try:
                ...
                print(f"Toast notification also failed: {toast_error}")
    
    def _show_fallback_break_completion(self):
        """Show fallback break completion notification when modal fails."""
        break_name = "Long Break" if self.current_session_type == SessionType.LONG_BREAK else "Short Break"
        break_emoji = "🌴" if self.current_session_type == SessionType.LONG_BREAK else "☕"
        ...
        if hasattr(self.notifier, 'show_toast_notification'):
            try:
                self.notifier.show_toast_notification(
                    ...
                    f"✨ Break over! Ready for another focus session? 🎯",
                    duration=10
                )
            except Exception as toast_error:
                print(f"Toast notification also failed: {toast_error}")
    ...
    def get_status(self) -> Dict:
        ...
        remaining_minutes = int(self.remaining_time // 60)  # Convert to int
        remaining_seconds = int(self.remaining_time % 60)   # Convert to int
